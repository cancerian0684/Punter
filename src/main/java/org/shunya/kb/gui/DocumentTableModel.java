...
import org.shunya.server.component.SearchResult;
...
public class DocumentTableModel extends AbstractTableModel {
    ...
    public DocumentTableModel(DBService dbService) {
        this.dbService = dbService;
    }

    public void setQuery(SearchQuery query) {
        this.query = query;
        this.query.maxResults = chunkSize;
        this.currentSize = 0;
        this.searchResult = dbService.getDocList(query);
        data.addAll(this.searchResult.getDocuments());
     ...
     */
    public void ensureCached(int row) {
        while (currentSize < row) {
            query.start = currentSize;
        ...
        switch (col) {
            case 0:
                ...
                if (doc.getDateCreated() != null) {
                    s1 = sdf.format(doc.getDateCreated());
                }
                String s2 = "";
